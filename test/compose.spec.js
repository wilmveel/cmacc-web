    ...
    describe('SAFE_Robinson_v2.md', function () {
        it('should parse variable hello World', function (done) {
            ...
            compose(file, null, function (err, ast) {
                ...
                var robinson = ast.variables[2].ast
                assert.equal(robinson.variables[2].key, "valuationCap");
                assert.equal(robinson.variables[2].val, "$1,000,000");

                assert.equal(robinson.variables[4].ast.variables[0].key, "fullName");
                assert.equal(robinson.variables[4].ast.variables[0].val, "Acme Inc.");

                assert.equal(robinson.variables[7].ast.variables[4].key, "Company");
                assert.equal(robinson.variables[7].ast.variables[4].val, "James");

                assert.equal(robinson.variables[8].ast.variables[0].ast.variables[4].key, "Company");
                assert.equal(robinson.variables[8].ast.variables[0].ast.variables[4].val, "James");

                var ycombinator = robinson.variables[9].ast;
                ...
                assert.equal(ycombinator.variables[6].ast.variables[4].key, "Company");
                assert.equal(ycombinator.variables[6].ast.variables[4].val, "James");
